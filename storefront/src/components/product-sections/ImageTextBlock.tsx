  ...
  return (
    <section className={`py-16 ${bgClass}`}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          {/* Image */}
          <div className={`relative aspect-[4/3] rounded-2xl overflow-hidden ${layout === "image-right" ? "lg:[direction:ltr]" : ""}`}>
            <Image
              src={image}
              alt={imageAlt || title}
              fill
          ...
          <div className={layout === "image-right" ? "lg:[direction:ltr]" : ""}>
            <h2 className={`text-3xl font-semibold ${textClass} mb-6`}>
              {title}
            </h2>
            {typeof body === "string" ? (
              <p className={`${bodyClass} text-lg leading-relaxed`}>{body}</p>
            ) : (
              <div className={`${bodyClass} text-lg leading-relaxed space-y-4`}>
                {body}
              </div>
            )}
          </div>
