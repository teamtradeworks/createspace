        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-navy rounded-2xl p-8 md:p-12">
            <div className="text-center mb-12">
              <h3 className="text-3xl font-semibold mb-4 text-white">{active.title}</h3>
              <p className="text-white/70 max-w-2xl mx-auto">{active.subtitle}</p>
            </div>
              ...
              {tabs.map((_, index) => (
                <button
                  ...
                  className={`w-2.5 h-2.5 rounded-full transition-all duration-200 ${
                    activeTab === index ? "bg-cs-orange w-8" : "bg-white/30 hover:bg-white/50"
                  }`}
                />
              ))}
            </div>
